...
class Output extends \Core\Controller{

    public function Send($strBufferedContent,$controllerobject)
    {
        $this->tpl->assign('site_url', SYS_SITE_URI);
		$this->tpl->assign('admin_url', SYS_SITE_URI.LOGINURL.'/');
        ...
        if(SYS_MINIFY==1){
            $htmlcontent=$this->tpl->fetch($wrapper);
            $minifiedcode=self::minify_html($htmlcontent);
            print_r($minifiedcode);
        }else{
